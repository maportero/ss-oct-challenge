...
if ( ! class_exists( 'xblog_About' ) ) {
	...
	class xblog_About {
		 ...
		 *
		 * @var xblog_About $instance The  TI_About_Page instance.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof xblog_About ) ) {
				self::$instance = new xblog_About;
		...
		public function ti_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap xblog-about-wrap">';
				...
				echo '<a href="'.esc_url('https://wpthemespace.com/product/x-blog/').'" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
		...
		public function get_plugin_icon( $arr ) {
			...
			if ( ! empty( $arr['svg'] ) ) {
				...
				$plugin_icon_url = get_stylesheet_directory_uri() . '/inc/about/images/logo.png';
			}
    ...
    return add_query_arg(
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section xblog-section xblog-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .xblog-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section xblog-section xblog-section-support three-col">
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="'.esc_url('https://wpthemespace.com/product/x-blog/').'" target="_blank" >';
					echo esc_html ('X Blog Pro Live Preview', 'x-blog');
					echo '</a></td></tr>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'about-page-css', get_stylesheet_directory_uri() . '/inc/about/css/about.min.css', array(),'2.3.2','all'  );
			wp_enqueue_script( 'xnotice-plus-js', get_stylesheet_directory_uri() . '/inc/about/js/notice.js', array('jquery'), '1.2.0', true );

			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {

				wp_enqueue_script( 'about-page-js', get_stylesheet_directory_uri() . '/inc/about/js/about.min.js', array( 'jquery' ) );
        ...
        'activating_string'        => __( 'Activating', 'x-blog-free' )
        )
    );
